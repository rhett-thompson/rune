...
Loop_Mode :: enum { None, Restart, Ping_Pong }
...
make :: proc(duration: f32, easing: Ease = ease_linear) -> Tween {
	...
	return Tween{duration = tween_duration, easing = tween_easing, mode = .None, direction = 1, running = true}
}
...
value_f32 :: proc(tween: ^Tween, start, target: f32) -> f32 { return lerp_f32(start, target, eased_progress(tween)) }
...
value_vec3 :: proc(tween: ^Tween, start, target: [3]f32) -> [3]f32 {
	t := eased_progress(tween)
	return {lerp_f32(start[0], target[0], t), lerp_f32(start[1], target[1], t), lerp_f32(start[2], target[2], t)}
}

value_color :: proc(tween: ^Tween, start, target: Color) -> Color {
	t := eased_progress(tween)
	return Color{lerp_f32(start.r, target.r, t), lerp_f32(start.g, target.g, t), lerp_f32(start.b, target.b, t), lerp_f32(start.a, target.a, t)}
}
...
ease_in_out_quad :: proc(t: f32) -> f32 { if t < 0.5 { return 2 * t * t }; return 1 - f32(math.pow(f64(-2 * t + 2), 2)) / 2 }
ease_in_cubic :: proc(t: f32) -> f32 { return t * t * t }
ease_out_cubic :: proc(t: f32) -> f32 { return 1 - (1 - t) * (1 - t) * (1 - t) }
ease_in_out_cubic :: proc(t: f32) -> f32 { if t < 0.5 { return 4 * t * t * t }; return 1 - f32(math.pow(f64(-2 * t + 2), 3)) / 2 }
ease_in_quart :: proc(t: f32) -> f32 { return t * t * t * t }
ease_out_quart :: proc(t: f32) -> f32 { return 1 - f32(math.pow(f64(1 - t), 4)) }
ease_in_out_quart :: proc(t: f32) -> f32 { if t < 0.5 { return 8 * t * t * t * t }; return 1 - f32(math.pow(f64(-2 * t + 2), 4)) / 2 }

ease_in_back :: proc(t: f32) -> f32 { c1 := f32(1.70158); c3 := c1 + 1; return c3 * t * t * t - c1 * t * t }
ease_out_back :: proc(t: f32) -> f32 { c1 := f32(1.70158); c3 := c1 + 1; return 1 + c3 * f32(math.pow(f64(t - 1), 3)) + c1 * f32(math.pow(f64(t - 1), 2)) }
...
ease_in_out_bounce :: proc(t: f32) -> f32 { if t < 0.5 { return (1 - ease_out_bounce(1 - 2 * t)) / 2 }; return (1 + ease_out_bounce(2 * t - 1)) / 2 }
...
clamp :: proc(value, minimum, maximum: f32) -> f32 { if value < minimum { return minimum }; if value > maximum { return maximum }; return value }
