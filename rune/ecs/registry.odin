...
register_builtin_components :: proc(registry: ^Component_Registry) -> bool {
	transform_registered := register_builtin_component(registry, "Transform", Transform, "Position, rotation, and scale for an entity")
	sprite_registered := register_builtin_component(registry, "SpriteRenderer", SpriteRenderer, "2D texture renderer")
	mesh_registered := register_builtin_component(registry, "MeshRenderer", MeshRenderer, "Primitive 3D mesh renderer")
	sphere_registered := register_builtin_component(registry, "SphereRenderer", SphereRenderer, "Sphere 3D renderer")
	model_registered := register_builtin_component(registry, "ModelRenderer", ModelRenderer, "Asset-backed 3D model renderer")
	ambient_light_registered := register_builtin_component(registry, "AmbientLight", AmbientLight, "Scene ambient light color and intensity")
	directional_light_registered := register_builtin_component(registry, "DirectionalLight", DirectionalLight, "Directional scene light for lit 3D materials")
	point_light_registered := register_builtin_component(registry, "PointLight", PointLight, "Local point light for lit 3D materials")
	spot_light_registered := register_builtin_component(registry, "SpotLight", SpotLight, "Cone-shaped local light for lit 3D materials")
	tilemap_registered := register_builtin_component(registry, "TilemapRenderer", TilemapRenderer, "Texture-atlas 2D tile grid")
	text_registered := register_builtin_component(registry, "TextRenderer", TextRenderer, "Scene-authored 2D text")
	tilemap_collider_registered := register_builtin_component(registry, "TilemapCollider", TilemapCollider, "Solid-tile collision for a TilemapRenderer")
	top_down_controller_registered := register_builtin_component(registry, "TopDownController", TopDownController, "2D tilemap collision controller")
	rigid_body_2d_registered := register_builtin_component(registry, "RigidBody2D", RigidBody2D, "Fixed-step 2D physics body")
	box_collider_2d_registered := register_builtin_component(registry, "BoxCollider2D", BoxCollider2D, "2D axis-aligned box collider")
	circle_collider_2d_registered := register_builtin_component(registry, "CircleCollider2D", CircleCollider2D, "2D circle collider")
	rigid_body_3d_registered := register_builtin_component(registry, "RigidBody3D", RigidBody3D, "Box3D-backed 3D rigid body")
	box_collider_registered := register_builtin_component(registry, "BoxCollider", BoxCollider, "Axis-aligned static collision volume")
	sphere_collider_registered := register_builtin_component(registry, "SphereCollider", SphereCollider, "Sphere-shaped static collision volume")
	character_controller_registered := register_builtin_component(registry, "CharacterController", CharacterController, "Gravity and collision player controller")
	orbit_registered := register_builtin_component(registry, "Orbit", Orbit, "Moves an entity around its parent on the XZ plane")
	rotator_registered := register_builtin_component(registry, "Rotator", Rotator, "Spins an entity around its local Y axis")
	camera_2d_registered := register_builtin_component(registry, "Camera2D", Camera2D, "2D view controlled by an entity Transform")
	camera_3d_registered := register_builtin_component(registry, "Camera3D", Camera3D, "3D view controlled by an entity Transform")
	orbit_camera_3d_registered := register_builtin_component(registry, "OrbitCamera3D", OrbitCamera3D, "Input-driven orbit controller for a Camera3D entity")
	audio_listener_registered := register_builtin_component(registry, "AudioListener", AudioListener, "Scene audio reference point, normally attached to the active camera")
	...
	nav_grid_2d_registered := register_builtin_component(registry, "NavGrid2D", NavGrid2D, "Scene-wide 2D navigation grid settings")
	nav_agent_2d_registered := register_builtin_component(registry, "NavAgent2D", NavAgent2D, "2D pathfinding agent settings")
	return transform_registered && sprite_registered && mesh_registered && sphere_registered && model_registered && ambient_light_registered && directional_light_registered && point_light_registered && spot_light_registered && tilemap_registered && text_registered && tilemap_collider_registered && top_down_controller_registered && rigid_body_2d_registered && box_collider_2d_registered && circle_collider_2d_registered && rigid_body_3d_registered && box_collider_registered && sphere_collider_registered && character_controller_registered && orbit_registered && rotator_registered && camera_2d_registered && camera_3d_registered && orbit_camera_3d_registered && audio_listener_registered && audio_player_registered && nav_grid_2d_registered && nav_agent_2d_registered
}
...
register_builtin_component :: proc(
	...
	return register_component(registry, Component_Descriptor{
		name = name,
...
register_component_descriptor :: proc(registry: ^Component_Registry, descriptor: Component_Descriptor) -> bool {
...
register_data_component :: proc(registry: ^Component_Registry, name: string, description := "") -> bool {
	return register_component(registry, Component_Descriptor{name = name, description = description})
}
...
register_component_type :: proc(
	...
	if registry == nil || registry.typed_defaults_arena == nil ||
	   len(name) == 0 || has_component(registry, name) || !is_struct_type(T) {
		return false
	}
	...
	if marshal_error != nil || json.unmarshal(default_bytes, defaults, allocator = allocator) != nil {
		return false
	}
...
register_component :: proc{register_component_descriptor, register_component_type}
...
typed_component_create_proc :: proc($T: typeid) -> Typed_Component_Create_Proc {
	return proc(data: json.Value, default_value: any, allocator: mem.Allocator) -> (any, bool) {
		...
		if default_marshal_error != nil || json.unmarshal(default_bytes, value, allocator = allocator) != nil {
			return nil, false
		}
