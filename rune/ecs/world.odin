...
apply_value_snapshot :: proc(world: ^World, snapshot: ^World) -> bool {
	...
	if !same_parent_shape(world, snapshot, entity_translation) ||
	   ...
	   !same_component_instance_shape(world.component_instance_data, snapshot.component_instance_data, entity_translation) {
		return false
	}
...
build_snapshot_entity_translation :: proc(world: ^World, snapshot: ^World, result: ^map[Entity]Entity) -> bool {
	if world.entity_count != snapshot.entity_count || len(world.entity_ids) != len(snapshot.entity_ids) {
		return false
	}
...
same_parent_shape :: proc(world: ^World, snapshot: ^World, entity_translation: map[Entity]Entity) -> bool {
	for snapshot_entity, target_entity in entity_translation {
...
same_component_shape :: proc(current, snapshot: map[string]map[Entity]json.Value, entity_translation: map[Entity]Entity) -> bool {
	...
	for name, snapshot_components in snapshot {
		...
		for snapshot_entity in snapshot_components {
			...
			if _, component_found := current_components[target_entity]; !component_found { return false }
		}
	}
	return true
}

same_component_instance_shape :: proc(current, snapshot: map[string]map[Component_Instance]json.Value, entity_translation: map[Entity]Entity) -> bool {
	if len(current) != len(snapshot) { return false }
	for name, snapshot_instances in snapshot {
		...
		for snapshot_key in snapshot_instances {
			...
			if !entity_found { return false }
			if _, instance_found := current_instances[Component_Instance{entity = target_entity, name = snapshot_key.name}]; !instance_found {
				return false
			}
		}
	}
...
apply_changed_component_values :: proc(world: ^World, snapshot: ^World, entity_translation: map[Entity]Entity) {
...
apply_changed_component_instance_values :: proc(world: ^World, snapshot: ^World, entity_translation: map[Entity]Entity) {
	for name, snapshot_instances in snapshot.component_instance_data {
		...
		for snapshot_key, snapshot_value in snapshot_instances {
			target_key := Component_Instance{entity = entity_translation[snapshot_key.entity], name = snapshot_key.name}
...
apply_snapshot_component_value :: proc(world: ^World, snapshot: ^World, target_entity, snapshot_entity: Entity, name: string) {
	...
	if name == "SphereRenderer" { world.sphere_renderers[target_entity] = snapshot.sphere_renderers[snapshot_entity] }
	if name == "ModelRenderer" { world.model_renderers[target_entity] = snapshot.model_renderers[snapshot_entity] }
	if name == "AmbientLight" { world.ambient_lights[target_entity] = snapshot.ambient_lights[snapshot_entity] }
	if name == "DirectionalLight" { world.directional_lights[target_entity] = snapshot.directional_lights[snapshot_entity] }
	if name == "PointLight" { world.point_lights[target_entity] = snapshot.point_lights[snapshot_entity] }
	if name == "SpotLight" { world.spot_lights[target_entity] = snapshot.spot_lights[snapshot_entity] }
	if name == "TilemapRenderer" { world.tilemap_renderers[target_entity] = snapshot.tilemap_renderers[snapshot_entity] }
	if name == "TextRenderer" { world.text_renderers[target_entity] = snapshot.text_renderers[snapshot_entity] }
	if name == "TilemapCollider" { world.tilemap_colliders[target_entity] = snapshot.tilemap_colliders[snapshot_entity] }
	if name == "TopDownController" { world.top_down_controllers[target_entity] = snapshot.top_down_controllers[snapshot_entity] }
	...
	if name == "OrbitCamera3D" { world.orbit_cameras_3d[target_entity] = snapshot.orbit_cameras_3d[snapshot_entity] }
	if name == "AudioListener" { world.audio_listeners[target_entity] = snapshot.audio_listeners[snapshot_entity] }
	if name == "NavGrid2D" { world.nav_grids_2d[target_entity] = snapshot.nav_grids_2d[snapshot_entity] }
	if name == "NavAgent2D" { world.nav_agents_2d[target_entity] = snapshot.nav_agents_2d[snapshot_entity] }
}
