...
character_intersects_sphere :: proc(world: ^World, controller: CharacterController, eye_position: [3]f32, collider_entity: Entity, collider: SphereCollider) -> bool {
	center, radius, bounds_ok := sphere_bounds(world, collider_entity, collider)
	if !bounds_ok { return false }
	character_min := [3]f32{eye_position[0] - controller.radius, eye_position[1] - controller.eye_height, eye_position[2] - controller.radius}
	character_max := [3]f32{eye_position[0] + controller.radius, eye_position[1] + (controller.height - controller.eye_height), eye_position[2] + controller.radius}
	...
	return difference[0] * difference[0] + difference[1] * difference[1] + difference[2] * difference[2] < radius * radius
}

sphere_bounds :: proc(world: ^World, entity: Entity, collider: SphereCollider) -> ([3]f32, f32, bool) {
	transform, found := get_transform(world, entity)
