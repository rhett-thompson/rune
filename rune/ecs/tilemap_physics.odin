...
top_down_controller_from_json :: proc(data: json.Value) -> (TopDownController, bool) {
	object, ok := data.(json.Object)
	if !ok { return {}, false }
	result := TopDownController{size = {16, 16}, speed = 180}
	if value, found := object["size"]; found && !read_vector2(value, &result.size) { return {}, false }
	if value, found := object["speed"]; found { result.speed, ok = read_number(value); if !ok || result.speed <= 0 { return {}, false } }
...
top_down_blocked :: proc(world: ^World, entity: Entity, controller: TopDownController, position: [2]f32) -> bool {
	player_min := [2]f32{position[0] - controller.size[0] * 0.5, position[1] - controller.size[1] * 0.5}
	player_max := [2]f32{position[0] + controller.size[0] * 0.5, position[1] + controller.size[1] * 0.5}
	for map_entity in entities_with_component(world, "TilemapCollider") {
		if !collides_by_layer(world, entity, map_entity) { continue }
		collider, has_collider := get_tilemap_collider(world, map_entity)
		tilemap, has_tilemap := get_tilemap_renderer(world, map_entity)
		map_transform, has_transform := get_transform(world, map_entity)
		if !has_collider || !has_tilemap || !has_transform { continue }
		for tile in tilemap.tiles {
			if !tile_is_solid(collider, tile.index) { continue }
			tile_min := [2]f32{
				map_transform.position[0] + f32(tile.x) * tilemap.tile_size[0] * map_transform.scale[0],
				map_transform.position[1] + f32(tile.y) * tilemap.tile_size[1] * map_transform.scale[1],
			}
			tile_max := tile_min + [2]f32{tilemap.tile_size[0] * map_transform.scale[0], tilemap.tile_size[1] * map_transform.scale[1]}
			if player_min[0] < tile_max[0] && player_max[0] > tile_min[0] && player_min[1] < tile_max[1] && player_max[1] > tile_min[1] {
				return true
			}
		}
	}
...
tile_is_solid :: proc(collider: TilemapCollider, index: i32) -> bool {
	for solid_index in collider.solid_tiles {
		if solid_index == index { return true }
	}
	return false
}
