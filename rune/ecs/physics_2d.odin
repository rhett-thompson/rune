...
physics_2d_update :: proc(world: ^World, dt: f32) {
	if dt <= 0 ||
	   (len(world.rigid_bodies_2d) == 0 &&
			   ...
			   len(world.circle_colliders_2d) == 0) {return}
...
create_box2d_static :: proc(world: ^World, entity: Entity) {
	transform, found := get_transform(
		world,
		entity,
	); collider, has_collider := world.box_colliders_2d[entity]
	if !found || !has_collider {return}
	mask, _ := entity_layer_mask(world, entity)
	body := create_box2d_body_id(
		world,
		RigidBody2D{gravity_scale = 0, body_type = "static"},
...
create_circle2d_static :: proc(world: ^World, entity: Entity) {
	transform, found := get_transform(
		world,
		entity,
	); collider, has_collider := world.circle_colliders_2d[entity]
	if !found || !has_collider {return}
	mask, _ := entity_layer_mask(world, entity)
