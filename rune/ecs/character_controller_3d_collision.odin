...
character_capsule_3d :: proc(radius,height: f32,up: [3]f32={0,1,0}) -> b3.Capsule {
	return {center1=character_vec_3d(up*radius),center2=character_vec_3d(up*(height-radius)),radius=radius}
}
...
character_plane_result_3d :: proc "c"(shape:b3.ShapeId,planes:[^]b3.PlaneResult,count:c.int,ctx:rawptr)->bool {
	...
	for i in 0..<int(count) {
		...
		normal := character_array_3d(plane.normal)
		vertical := character_dot_3d(normal,query.up)
		if vertical > 0.01 && vertical < query.min_up {
			tangent := normal-query.up*vertical
			horizontal:=character_vec_3d(tangent/character_length_3d(tangent))
...
character_collect_planes_3d :: proc(query:^Character_Plane_Query_3D,position:[3]f32,radius,height:f32) {
	...
	capsule := character_capsule_3d(radius,height,query.up)
...
character_sweep_3d :: proc(world:^World,entity:Entity,position,delta:[3]f32,radius,height:f32,up:[3]f32={0,1,0})->(Raycast_Hit_3D,bool) {
	points := [2]b3.Vec3{character_vec_3d(up*radius),character_vec_3d(up*(height-radius))}
...
character_slide_3d :: proc(query:^Character_Plane_Query_3D,position,translation:[3]f32,radius,height:f32)->[3]f32 {
	...
	for _ in 0..<6 {
		...
		fraction := b3.World_CastMover(query.world.box3d_world,{result[0],result[1],result[2]},
			character_capsule_3d(radius,height,query.up),character_vec_3d(delta),character_filter_3d(query.world,query.entity),
...
character_ground_3d :: proc(world:^World,entity:Entity,position:[3]f32,radius,height,distance,min_up:f32,up:[3]f32={0,1,0})->(Raycast_Hit_3D,[3]f32,bool) {
...
character_try_step_3d :: proc(query:^Character_Plane_Query_3D,start,translation,ordinary:[3]f32,config:CharacterController3D,height,min_up:f32)->([3]f32,bool) {
	...
	horizontal := translation-query.up*character_dot_3d(translation,query.up)
	...
	lift := query.up*config.step_height
	if hit,blocked := character_sweep_3d(query.world,query.entity,start,lift,config.radius,height,query.up); blocked && hit.fraction < 1 {return ordinary,false}
	raised := start+lift
...
character_clearance_3d :: proc(world:^World,entity:Entity,position:[3]f32,radius,height:f32,up:[3]f32={0,1,0})->bool {
	points := [2]b3.Vec3{character_vec_3d(up*radius),character_vec_3d(up*(height-radius))}
