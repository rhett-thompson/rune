...
load_pong_game :: proc(game: ^Pong_Game, world: ^ecs.World) -> bool {
	...
	if !ok_a ||
	   ...
	   !ok_1 ||
	   !ok_2 ||
	   first.computer == second.computer {return false}
...
paddle_hit :: proc(game: ^Pong_Game, paddle: Pong_Paddle, moving_left: bool) -> bool {
	b := game.ball
	if moving_left {
		if b.position.x - b.radius > paddle.x + f32(paddle.width) / 2 ||
		   b.position.x < paddle.x {return false}
	} else if b.position.x + b.radius < paddle.x - f32(paddle.width) / 2 ||
	   b.position.x > paddle.x {return false}
	return(
...
bounce :: proc(game: ^Pong_Game, paddle: Pong_Paddle, right: bool) {
	...
	speed := clamp(
		ball_speed(game.ball) + game.ball.speed_increase,
		game.ball.start_speed,
		game.ball.max_speed,
	)
...
update_pong :: proc(game: ^Pong_Game, engine: ^rune.Engine) {
	...
	move_paddle(
		&game.left,
		input.axis(controls, game.left.input_axis),
		engine.delta_time,
		game.arena,
	)
	if game.match.two_player {
		move_paddle(
			&game.right,
			input.axis(controls, game.right.input_axis),
			engine.delta_time,
			game.arena,
		)
	} else {
...
draw_pong :: proc(game: ^Pong_Game) {
		...
		rl.DrawRectangleLinesEx(
			{
				...
				f32(a.border) - outset,
				outset * 2 + 2,
				f32(a.height - a.border * 2) + outset * 2,
			},
			3,
			red,
		)
	...
	for y := a.border + 10;
	    y < a.height - a.border - 10;
	    y += 24 {rl.DrawRectangle(a.width / 2 - 2, y, 4, 13, line)}
	...
	rl.DrawRectangle(
		i32(game.right.x) - game.right.width / 2,
		i32(game.right.y) - game.right.height / 2,
		game.right.width,
		game.right.height,
		color(a.right_color),
	)
	...
	if game.match.game_over {
		winner :=
			"LEFT PLAYER WINS" if game.match.left_score > game.match.right_score else ("RIGHT PLAYER WINS" if game.match.two_player else "COMPUTER WINS")
