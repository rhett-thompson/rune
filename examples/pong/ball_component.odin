...
ball_from_entity :: proc(world: ^ecs.World, entity: ecs.Entity) -> (Pong_Ball, bool) {
	...
	ok := found && result.radius > 0 && result.start_speed > 0 && result.max_speed >= result.start_speed
	return result, ok
}
...
ball_speed :: proc(ball: Pong_Ball) -> f32 {
	return f32(math.sqrt(f64(ball.velocity.x * ball.velocity.x + ball.velocity.y * ball.velocity.y)))
}
