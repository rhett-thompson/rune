...
start :: proc(game: ^rune.Engine, world: ^ecs.World) {
	if !bridge.initialized {
		ok: bool
		bridge, ok = r3d_bridge.init(
			"examples/skeletal_animation_3d",
			rl.GetScreenWidth(),
			rl.GetScreenHeight(),
		)
...
draw :: proc(game: ^rune.Engine, world: ^ecs.World) {
	r3d_bridge.draw_scene_ex(
		&bridge,
		world,
		rune.asset_manager(game),
		{grid_slices = 16, grid_spacing = 1},
	)
	rl.DrawText("SKELETAL ANIMATION", 30, 25, 28, rl.RAYWHITE)
	rl.DrawText(
		"Shared model, independent R3D players: bend 1x / bend 0.5x / sway",
		30,
		66,
		18,
		rl.LIGHTGRAY,
