...
ui_update :: proc(game: ^rune.Engine, world: ^ecs.World) {
	...
	ui.panel(&interface, "screen", {
		layout = {sizing = {width = ui.grow({}), height = ui.grow({})},
			padding = ui.padding(24), childAlignment = {x = .Center, y = .Center}},
		backgroundColor = {9, 14, 24, 165} if menu_open else {},
	})
	if menu_open {
		ui.panel(&interface, "menu", {
			layout = {sizing = {width = ui.grow({max = 420}), height = ui.fit({})},
				layoutDirection = .TopToBottom, padding = ui.padding(28), childGap = 20},
			backgroundColor = {20, 29, 44, 250}, cornerRadius = ui.corners(12),
			border = {color = {48, 66, 88, 255}, width = {left=1,right=1,top=1,bottom=1}},
		})
		ui.label(&interface, "RUNE / UI DEMO", interface.style.accent, 14)
		ui.label(&interface, "Paused", interface.style.text, 38)
		ui.label(&interface, "Your game will be here when you're ready.", interface.style.muted, 18)
		...
		ui.panel(&interface, "hint", {
			layout = {sizing = {width = ui.fit({}), height = ui.fit({})}, padding = ui.padding(18)},
			floating = {attachTo = .Root, attachment = {element = .CenterBottom, parent = .CenterBottom}, offset = {0,-24}},
			backgroundColor = {20, 29, 44, 230}, cornerRadius = ui.corners(8),
		})
...
ui_status :: proc(dev: ^console.Console, arguments: string) {
	focus := "resume" if ui.focused(&interface,"resume") else "volume" if ui.focused(&interface,"volume") else "quit" if ui.focused(&interface,"quit") else ""
	console.set_result(dev, struct{open:bool, volume:f32, focus:string, layout_error:string}{menu_open,volume,focus,ui.last_error(&interface)})
}
...
main :: proc() {
	...
	if !rune.register_system(&game, {name="clay_menu",start=start,ui_update=ui_update,
		update=update,pre_draw=background,draw_ui=draw,shutdown=shutdown}) {return}
