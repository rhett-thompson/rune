...
update_footsteps :: proc(game: ^rune.Engine, world: ^ecs.World) {
	...
	// Measure resolved ground travel, excluding passive platform carry. Camera
	// bob can be disabled independently without silencing walking sounds.
...
on_post_physics :: proc(game: ^rune.Engine, world: ^ecs.World) {
	update_weapon_motion(game, world)
	update_footsteps(game, world)
}
