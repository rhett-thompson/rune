...
validate_cached_state :: proc(registry: ^ecs.Component_Registry) {
	...
	add(&snapshot, registry, other, "AudioPlayer", `{"one":{"sound":"one.wav","volume":0.3},"two":{"clips":["two.wav","three.wav"],"volume":0.4}}`)
	version = ecs.change_version(&world)
	assert(ecs.apply_value_snapshot(&world, &snapshot))
	ecs.destroy(&snapshot)
	...
	assert(player.volume == 0.4 && len(player.clips) == 2 && player.clips[1] == "three.wav")
	assert(ecs.set_runtime_field(&world, registry, entity, "AudioPlayer", "two.clips.1", parse(`"four.wav"`)))
	player, _ = ecs.get_audio_player(&world, entity, "two")
	assert(player.clips[1] == "four.wav")
	version = ecs.change_version(&world)
